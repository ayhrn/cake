...
//debug($result); /*debug($entryCount);*/ die();
	...
	public function testFormWithValidFile() {
		...
		$stub = $this->getMockForModel('Contact', array('is_uploaded_file','move_uploaded_file'));
...
// modified with windows DS backslash
    				'tmp_name' => 'C:\uniformServer_11_7_8\UniServerZ\tmp\TestFile.jpg',
...
//    				'tmp_name' => ROOT.DS.APP_DIR.DS.'Test'.DS.'Case'.DS.'Model'.DS.'TestFile.jpg',
		...
		// Get the count in the DB
		$entryCount = $this->Contact->find('count',
                 array(
			'conditions' => array(
		'Contact.name' => 'James Fairhurst',
		'Contact.email' => 'info@jamesfairhurst.co.uk',
	...
	public function testFormWithInvalidFile() {
		...
		$data = array('Contact' => array(
			...
			'filename' => array(
				'name' => 'TestFile.txt',
				'type' => 'text/plain',
				'tmp_name' => 'C:\uniformServer_11_7_8\UniServerZ\tmp\TestFile.txt',
				'error' => 0,
				'size' => 19,
			)
		));
